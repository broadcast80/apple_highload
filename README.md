...
|Playlist_Song| Cassandra | playlist_id - поиск трека в конкретном плейлисте |
|LikedSongs| Cassandra | user_id - при просмотре добавленных треков |
...
- По id пользователя (User) - шардинг на основе уникального идентификатора пользователя. Это позволит распределить данные по нескольким шардам, улучшая масштабируемость и производительность.
- По id трека (Song) - шардинг на основе уникального идентификатора трека.
...
## 7. Алгоритмы
|Алгоритм |Где применяется |Мотивация |
| ------------- |-------------|-------------|
...
| ------------- |-------------|-------------|
|Golang |бекенд| высокая производительность, многопоточность, большое число библиотек |
...
|Kafka |стриминговая БД, сбор большого потока событий-данных |  |
